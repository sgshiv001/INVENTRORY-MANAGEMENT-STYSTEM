...
def write_csv(path: Path) -> None:
    ...
    with path.open("w", newline="", encoding="utf-8") as file:
        ...
        writer.writerow(["Name", "SKU", "Category", "Quantity", "Reorder Level", "Cost", "Price", "Value", "Margin", "Supplier", "Status"])
...
def write_summary(path: Path) -> None:
    ...
    if low_stock:
        lines.extend(f"- {product['name']} ({product['sku']}): {product['quantity']} left, reorder level {product['reorder']}" for product in low_stock)
    else:
